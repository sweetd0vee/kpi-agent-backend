...
from src.db.models import PprRow
from src.models.goal_tables import GoalRow, GoalTable

router = APIRouter()


def _db_to_schema(row: PprRow) -> GoalRow:
    return GoalRow(
        id=row.id,
...
def _schema_to_db(row: GoalRow) -> PprRow:
    return PprRow(
        id=row.id,
...
def get_ppr_table(db: Session = Depends(get_db)):
    rows = db.query(PprRow).order_by(PprRow.id).all()
    return GoalTable(rows=[_db_to_schema(row) for row in rows])


@router.put("", response_model=GoalTable)
def replace_ppr_table(payload: GoalTable, db: Session = Depends(get_db)):
    unique_rows = _dedupe_rows(payload.rows)
    try:
        db.query(PprRow).delete()
        if unique_rows:
