...
Таблицы создаются автоматически при старте приложения (`init_db`) или вызовом `POST /api/db/init`.
...
│ process_name            STR   Наименование    │
│ process_owner           STR   Владелец        │
│ owner_full_name_ref     STR   Справочно ФИО   │
│ business_unit           STR   Бизнес/блок     │
│ top_20                  STR   ТОП 20           │
└───────────────────────────────────────────────┘
...
│ functional_block        STR   Функц. блок     │
...
### 1. `leaders` — Справочник руководителей

| Колонка     | Тип    | Ограничения       | Описание                |
|-------------|--------|--------------------|-------------------------|
| `id`        | UUID   | PK, auto-generated | Уникальный идентификатор |
| `full_name` | STRING | NOT NULL, UNIQUE   | ФИО руководителя        |

Справочник, на который ссылаются цели правления через `leader_id`.

### 2. `board_goals` — Цели правления

Объединённая таблица (ранее были отдельные `kpi` и `ppr`).

| Колонка        | Тип         | Ограничения      | Описание                                        |
|----------------|-------------|-------------------|-------------------------------------------------|
| `id`           | VARCHAR(64) | PK               | Уникальный идентификатор строки                 |
| `last_name`    | STRING      | NOT NULL         | ФИО руководителя (текстовое дублирование)       |
| `leader_id`    | UUID        | FK → leaders.id  | Ссылка на справочник руководителей (может быть NULL); при сохранении API подставляет по совпадению `last_name` с `leaders.full_name` |
...
**Связь**: `board_goals.leader_id` → `leaders.id` (many-to-one, опциональная).
...
### 3. `leader_goals` — Цели руководителей
...
### 4. `strategy_goals` — Цели стратегии
...
### 5. `process_registry` — Реестр процессов
...
| `process_name`        | STRING      | NOT NULL    | Наименование процесса                            |
| `process_owner`       | STRING      | NOT NULL    | Владелец процесса                                |
| `owner_full_name_ref` | STRING      | NOT NULL    | Справочно (ФИО владельца процесса)               |
| `business_unit`       | STRING      | NOT NULL    | Бизнес/блок (колонка «Блок» в шаблоне)           |
| `top_20`              | STRING      | NOT NULL    | ТОП 20 (текст/признак, например «Да»/«Нет»)      |

Загрузка из Excel: `POST /api/process-registry/upload` (см. раздел «API-эндпоинты»). Заголовки в xlsx должны соответствовать колонкам выше (допускаются синонимы в коде `xlsx_process_registry_import.py`).
...
### 6. `staff` — Оргструктура (штат)

Справочник подразделений: код оргструктуры, наименование, руководитель, функциональный блок и куратор. FK к другим таблицам не используется.
...
| `functional_block`         | STRING      | NOT NULL    | Функциональный блок                    |
...
```
leaders (1) ──────FK──────< (N) board_goals
   │
   │  leaders.id = board_goals.leader_id
   │  Один руководитель может иметь много целей правления.
   │  leader_id — опциональный (NULL допустим).
   │
   └─ Справочник ФИО. Используется для нормализации: вместо
      дублирования строки ФИО в каждой строке целей,
      можно ссылаться на leader_id.
...
leader_goals ── независимая таблица
   ...
   │  (FK к leaders пока не установлен, связь логическая.)
...
| `leaders`        | `GET /api/leaders`     | `POST /api/leaders`    | — |
...
Для **реестра процессов** первая строка листа — заголовки (регистр и лишние пробелы допускаются). Распознаваемые имена колонок: «Процессная область», «Код процесса», «Наименование процесса» (или «Наименование»), «Владелец процесса», «Справочно ФИО» / «ФИО владельца», «Бизнес/блок» или «Блок», «ТОП 20». Колонка «id» необязательна: если её нет, для каждой строки генерируется идентификатор как на фронте.
...
- **id** в `board_goals`, `leader_goals`, `strategy_goals`, `process_registry`, `staff` — строка формата `{timestamp}-{random}`, генерируемая на фронтенде.
- **leaders.id** — UUID v4, генерируется на бэкенде при создании руководителя.
- **leader_id** в `board_goals` — опциональная FK-ссылка (UUID). Если не указан — NULL.
- **Каскадное удаление** не настроено: при удалении руководителя из `leaders` строки `board_goals` с его `leader_id` **не** удаляются.
